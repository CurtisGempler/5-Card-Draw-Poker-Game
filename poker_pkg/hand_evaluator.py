...
def evaluate_hand(player) -> None:
    ...
    unsuited_hand = player.unsuited_cards
    suited_hand = player.suited_cards
    ...
    def set_hand(hand_string, hand_value):
        player.hand_string = hand_string
    ...
    if is_flush() and is_straight():
        ...
        set_hand(f"Flush {card_value_translator[hand_high_card]} high", 6)
    elif is_straight():
        ...
        max_cards = card_occurance(unsuited_occurance)
        set_hand(f"Two Pair {card_value_translator[max_cards[0]]}'s and {card_value_translator[max_cards[1]]}'s", 3)
    elif is_pair():
        max_cards = card_occurance(unsuited_occurance)
        set_hand(f"Pair {card_value_translator[max_cards[0]]}'s", 2)
    else:
...
def hand_comparision(players: dict) -> list:
    ...
    def get_sorted_unsuited_hand(player) -> list:
        return sorted(player.unsuited_cards, reverse = True)
    ...
    high_hand_value = players["Player 1"].hand_value

    for player_name, player in players.items():

        if player_name == "Player 1" or player.hand_value == 0:
            continue
        ...
        if high_hand_value < player_hand_value:
            ...
            # Royal Flush suit comparison in event of matching hand values
                ...
                high_hand_suits = players[high_hand_player[0]].suited_cards
                player_hand_suits = player.suited_cards
                suits = {"♠️": 1,"♥️": 2,"♦️": 3,"♣️": 4}
                if suits[high_hand_suits[0]] < suits[player_hand_suits[0]]:
                    set_high_hand_player(1, player_name)

        high_hand_value = players[high_hand_player[0]].hand_value
